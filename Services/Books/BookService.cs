...
namespace Services.Books
{
    public class BookService(IBookApiService booksApiBLL, IBookRepo bookDAL) : IBookService
    {
        ...
        public async Task<ServiceResponse> UpdateAsync(int uid, bool isOn, Book book)
        {
            ...
            else return new ServiceResponse() { Success = false, Content = "Livro com este título já cadastrado." };
        }

        public async Task<ServiceResponse> AddAsync(int uid, bool isOn, Book book)
        {
            book.UpdatedAt = DateTime.Now;
            book.UserId = uid;
            ...
            if (bookResponse is null)
            {
                ...
                if (isOn)
                {
                    ServiceResponse response = await booksApiBLL.CreateAsync(book);
                    ...
                    else
                    {
                        ...
                        if (response.Content is not null)
                            return new ServiceResponse() { Success = false, Content = response.Content.ToString() };
                        else return new ServiceResponse() { Success = false };
                    }
                }
                else
                {
        ...
        private async Task ApiUpdateBook(Book book, bool isOn)
        {
            if (isOn)
            {
                ServiceResponse resp = await booksApiBLL.UpdateAsync(book);
